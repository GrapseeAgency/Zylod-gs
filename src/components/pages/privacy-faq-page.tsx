...
export function PrivacyFaqPage() {
  ...
  useEffect(() => {
    fetch('/api/legal/faq?docType=privacy_policy')
      .then(res => res.json())
      .then(res => {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-slate-50">
      <div className="md:hidden sticky top-0 z-20 bg-white border-b border-gray-100 px-4 py-3.5 flex items-center gap-3 shadow-sm">
        ...
        <div>
          <h1 className="font-bold text-gray-900 text-base md:text-2xl">Privacy Policy FAQ</h1>
          <p className="text-xs text-gray-400">Questions about Data Governance, Security & Rights</p>
        </div>
      </div>
      ...
      <div className="flex-1 max-w-2xl mx-auto w-full px-4 py-5 space-y-4 pb-24 md:px-6 md:py-8 md:space-y-6 md:pb-10 lg:max-w-4xl">
        ...
        <Button onClick={() => navigate('privacy-policy')} className="w-full md:w-auto md:px-10 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-bold py-3">
