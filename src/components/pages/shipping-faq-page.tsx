...
export function ShippingFaqPage() {
  ...
  useEffect(() => {
    fetch('/api/legal/faq?docType=shipping_policy')
      .then(res => res.json())
      .then(res => {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-slate-50">
      <div className="md:hidden sticky top-0 z-20 bg-white border-b border-gray-100 px-4 py-3.5 flex items-center gap-3 shadow-sm">
        ...
        <div>
          <h1 className="font-bold text-gray-900 text-base">Shipping & Logistics FAQ</h1>
          <p className="text-xs text-gray-400">Timelines, Courier Partners, Freight Slabs & Tracking</p>
        </div>
      </div>
      <h1 className="hidden md:block text-2xl font-bold text-slate-900">Shipping & Logistics FAQ</h1>
      ...
      <div className="flex-1 max-w-2xl mx-auto w-full px-4 py-5 space-y-4 pb-24">
        ...
        <Button onClick={() => navigate('shipping-policy')} className="w-full bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-bold py-3">
...
export default ShippingFaqPage
