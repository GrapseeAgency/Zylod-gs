...
export function WholesaleFaqPage() {
  ...
  useEffect(() => {
    fetch('/api/legal/faq?docType=wholesale_terms')
      .then(res => res.json())
      .then(res => {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-slate-50">
      <div className="md:hidden sticky top-0 z-20 bg-white border-b border-gray-100 px-4 py-3.5 flex items-center gap-3 shadow-sm">
        ...
        <div>
          <h1 className="font-bold text-gray-900 text-base">Wholesale Trade FAQ</h1>
          <p className="text-xs text-gray-400">MOQ, Custom Quotations, Factory Direct Vetting</p>
        </div>
      </div>
      <h1 className="hidden md:block text-2xl font-bold text-slate-900">Wholesale Trade FAQ</h1>
