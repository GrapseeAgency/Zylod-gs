...
export function PaymentFaqPage() {
  ...
  useEffect(() => {
    fetch('/api/legal/faq?docType=payment_terms')
      .then(res => res.json())
      .then(res => {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-slate-50">
      ...
      <div className="flex-1 max-w-2xl lg:max-w-4xl mx-auto w-full px-4 py-5 md:px-6 md:py-8 space-y-4 md:space-y-6 pb-24 md:pb-8">
        {loading ? (
        ...
        <Button onClick={() => navigate('payment-terms')} className="w-full md:w-auto md:px-8 bg-violet-600 hover:bg-violet-700 text-white rounded-xl text-xs font-bold py-3">
