...
enum WebShellScripts {
    ...
    /// F1+F4 — THE authoritative verification probe (Android
    /// WebShellScripts.documentVerifyScript parity). One round-trip returns
    /// everything the shell needs to decide whether a document may be shown:
    ///  - `identity` — the served bundle's build identity
    ///    (window.__ZylodBundleIdentity, set inline in <head>). F1: missing
    ///    identity = unknown bundle = never accepted.
    ///  - `spaReady` — the bundle supports the soft-navigate + ack contract.
    ///  - `page`/`params` — the SPA's LIVE committed page state. F4: the
    ///    shell reveals a destination ONLY when this matches the requested
    ///    pageId+params for the current navigation generation — pushState
    ///    "ok" and didFinish are never proof on their own.
    static let documentVerifyScript = """
        (function(){
    ...
    /// `documentVerifyScript` verification (ack-driven or polled).
